  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300 text-slate-900 dark:text-white">
        ...
        <main className="flex-1 min-h-screen">
          {/* Mobile header */}
          <div className="md:hidden sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/5 px-4 py-3 flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="text-slate-800 dark:text-white p-1">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
              </svg>
            </button>
            <span className="font-bold gradient-text">Deneme Takip</span>
            {student && <span className="ml-auto text-sm text-slate-500 dark:text-slate-400">{student.name}</span>}
          </div>
